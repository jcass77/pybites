...
def float_series() -> pd.Series:
    ...
    """
    return pd.Series((x / 1_000 for x in range(1_001)))
...
def alpha_index_series() -> pd.Series:
    ...
    """
    return pd.Series({v: i for i, v in enumerate(string.ascii_lowercase, start=1)})
...
def object_values_series() -> pd.Series:
    ...
    Don't worry about the series name.
    """
    return pd.Series({i: v for i, v in enumerate(string.ascii_uppercase, start=101)})
